...
# 1) 데이터 로드
data = pd.read_csv(
    "data/02_cleaned/clothes/0.onepiece_only.csv",
    encoding="utf-8-sig",
)
data_val = pd.read_csv(
    "data/02_cleaned/clothes_val/0.onepiece_only.csv",
    encoding="utf-8-sig",
)
data = pd.concat([data, data_val], axis=0).reset_index(drop=True)

data.rename(columns={"서브스타일": "원피스_서브스타일"}, inplace=True)

# 카테고리 점프수트 제거
data = data[data["원피스_카테고리"] != "점프수트"].reset_index(drop=True)
...
joblib.dump(
    ensemble, "code/2_data analysis/clothes/1_style_model.pkl"
)
joblib.dump(
    encoder, "code/2_data analysis/clothes/1_clothes_encoder.pkl"
)
joblib.dump(
    le, "code/2_data analysis/clothes/1_style_label_encoder.pkl"
)
